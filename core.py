...
def calculate_region_contacts(region1, region2, box, cutoff=6.0):
    """Calculate if two regions have contact (binary detection, LIPAC-compatible)

    Returns True if ANY residue pair between the two regions has contact within cutoff.
    This matches LIPAC's residue-level contact detection logic.

    Parameters
    ----------
    region1, region2 : MDAnalysis.AtomGroup
        Region selections
    box : array-like
        Box dimensions for PBC
    cutoff : float
        Contact cutoff distance in Angstrom (default: 6.0)
    ...
    try:
        # Optimization: COM distance pre-screening (same as LIPAC)
        ...
        # Check all residue pairs (LIPAC-compatible logic)
        for res1 in region1.residues:
            for res2 in region2.residues:
                ...
                # Optimization: residue COM pre-screening (same as LIPAC)
                ...
                # Check ALL atoms (no [:5] limitation - LIPAC uses all atoms)
                for atom1 in res1.atoms:
                    for atom2 in res2.atoms:
                        ...
                        # Early termination if contact found
                        if min_dist <= cutoff:
                            break
                ...
                # If ANY residue pair has contact, return True immediately
                if min_dist <= cutoff:
                    return True
    ...
    try:
        ...
        for i in range(len(protein_names)):
            for j in range(i + 1, len(protein_names)):
                ...
                # Binary contact detection (LIPAC-compatible)
                region_1_contacts = False
                region_2_contacts = False
