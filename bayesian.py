...
def plot_bayesian_results(bayesian_results: Dict, config: AnalysisConfig):
    ...
    if not bayesian_results:
        print("    WARNING: No Bayesian results to plot")
        return
    ...
    print(f"    Plotting Bayesian results for {target_pair}")
    print(f"    Available keys in pair_data: {list(pair_data.keys())}")
    ...
    try:
        ...
        counts, bins, patches = ax.hist(dimer_samples, bins=50, density=True, 
                                       alpha=0.7, color='#4C72B0', edgecolor='none')
    ...
    # PLOT 3: Bayesian energy comparison (regions) - REQUIRED TWO-PANEL PLOT
    print(f"    Checking for regional data...")
    print(f"    'region_1' in pair_data: {'region_1' in pair_data}")
    print(f"    'region_2' in pair_data: {'region_2' in pair_data}")

    if 'region_1' in pair_data and 'region_2' in pair_data:
        print(f"    Generating regional energy comparison plot...")
        try:
            ...
            # Histogram for region 1 (TM_N_term)
            ax1.hist(r1_samples, bins=30, alpha=0.6, color=region1_color, label=region1_name, edgecolor='black', linewidth=0.5)
            # Histogram for region 2 (JM)
            ax1.hist(r2_samples, bins=30, alpha=0.6, color=region2_color, label=region2_name, edgecolor='black', linewidth=0.5)
            ...
            ax1.set_xlabel('Free Energy (kJ/mol)', fontsize=12)
            ax1.set_ylabel('Frequency', fontsize=12)
            ax1.set_title(f'Region-Specific Free Energy Distributions: {target_pair}', fontsize=14, fontweight='bold')
            ax1.legend(fontsize=11)
            ax1.grid(True, linestyle='--', alpha=0.3)

            # Right panel: Difference Distribution (N-C)
            if 'difference' in pair_data:
                ...
                # Histogram for difference distribution
                ax2.hist(diff_samples, bins=30, alpha=0.7, color='#4DBBD5', edgecolor='black', linewidth=0.5)
                ...
                ax2.set_xlabel('Energy Difference (kJ/mol)', fontsize=12)
            ...
            plt.close()
            print(f"    Bayesian energy comparison plot saved (TWO-PANEL)")
        ...
        print(f"    WARNING: Regional data not found in pair_data!")
        print(f"    Cannot generate regional energy comparison plot")
        if 'region_1' not in pair_data:
            print(f"    Missing 'region_1' data")
        if 'region_2' not in pair_data:
            print(f"    Missing 'region_2' data")